...
rewards <- matrix(rep(0, 15), nrow=3)
rewards[1, 1] <- 10
rewards[1, 2] <- -1
rewards[1, 3] <- -1
rewards[1, 4] <- -1
...
act <- function(action, state) {
  ...
  new.state <- state
  #
  if(state["x"] == 1 && state["y"] == 1 || state["x"] == 3 && state["y"] == 3)
    return(state)
  #
...
value.iteration <- function(states, actions, rewards, values, gamma, niter) {
  for (j in 1:niter) {
    for (i in 1:nrow(states)) {
      ...
      if(i %in% c(1, 13)) next # terminal states
...
final.values <- value.iteration(states=states, actions=actions, rewards=rewards, values=values, gamma=1, niter=10)
